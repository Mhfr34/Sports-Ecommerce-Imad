...
const ChangeUserRole = ({ name, email, role, onClose, userId, callFunc }) => {
  ...
  const updateUserRole = async () => {
    ...
    console.log("role updated to:", dataResponse);
  };

  // Sort the roles to make the current role appear first
  const sortedRoles = [role, ...Object.values(ROLE).filter((el) => el !== role)];
